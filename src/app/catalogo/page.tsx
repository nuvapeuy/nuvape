...
import { MOCK_PRODUCTS, flagLabel } from "@/lib/mock-data";
...
const ALL_BRANDS = Array.from(new Set(MOCK_PRODUCTS.map((p) => p.brand)));
const ALL_FLAVORS = Array.from(new Set(MOCK_PRODUCTS.flatMap((p) => p.flavors)));
const ALL_FLAGS = Array.from(new Set(MOCK_PRODUCTS.flatMap((p) => p.flags)));
...
function CatalogoContent() {
  ...
  }, [query, brands, flavors, flags, maxPrice, onlyInStock]);
          ...
          <FilterGroup title="Sabores">
            {ALL_FLAVORS.map((f) => (
              <FilterCheckbox
                ...
                onChange={() => toggle(flavors, setFlavors, f)}
              />
            ))}
          </FilterGroup>
  ...
  return (
    <div className="glass relative flex items-center gap-5 overflow-hidden rounded-2xl p-5">
      ...
      <div className="flex-1">
        ...
        <p className="mt-2 text-2xl font-extrabold text-[var(--neon-purple)]">{price}</p>
      </div>
    </div>
  );
}
